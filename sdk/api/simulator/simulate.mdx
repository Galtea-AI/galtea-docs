...
<ResponseField name="agent_goes_first" type="bool">
  ...
  <Note>
    When `agent_goes_first` is set to `True`, the first call to your agent will have an empty `messages` list in the `AgentInput`. Ensure your agent's `call` method can handle cases where `input_data.last_user_message_str()` returns `None`.
  </Note>
</ResponseField>