---
title: 'Test Case Service'
description: "Test Case Service API methods in the Galtea SDK"
---

The Test Case Service in the Galtea SDK allows you to manage [test-cases](/concepts/product/test/case) for evaluating your products.
This Service is exposed by the `galtea.test_cases` object.

<Info>
  Remember that we will be using the `galtea` object. More information [here](/sdk/api/galtea).
</Info>

## SDK Methods
- [Create Test Case](/sdk/api/test-case-service/create)
- [Listing Test Cases](/sdk/api/test-case-service/list)
- [Retrieving Test Case](/sdk/api/test-case-service/get)
- [Deleting Test Case](/sdk/api/test-case-service/delete)
