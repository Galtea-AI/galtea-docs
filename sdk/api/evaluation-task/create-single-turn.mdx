...
evaluation_tasks = galtea.evaluation_tasks.create_single_turn(
    ...
    metrics=[
        EvaluationMetric(name="coherence"), # Standard metric
...
```python
from galtea import EvaluationMetric
...
evaluation_tasks = galtea.evaluation_tasks.create_single_turn(
    ...
    metrics=[EvaluationMetric(name="relevance"), EvaluationMetric(name="non-toxic")],
...
<ResponseField name="metrics" type="List[EvaluationMetric | CustomScoreEvaluationMetric]" required>
  A list of metrics to use for evaluation. You can provide:
  - Standard metrics as `EvaluationMetric` objects.
