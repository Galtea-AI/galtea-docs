...
You can run [evaluation tasks](/concepts/product/evaluation/task) directly by providing a `version_id`, which automatically creates a session to store the inference results:
...
```python
# Get tasks for a specific evaluation
tasks = galtea.evaluation_tasks.list(evaluation_id="YOUR_EVALUATION_ID")

# Or get tasks for a specific session
tasks = galtea.evaluation_tasks.list(session_id="YOUR_SESSION_ID")
