...
```
<Note>
  This workflow can also be used for production monitoring by creating a session with `is_production=True` and omitting the `test_case_id`. See the [Monitor Production Responses](/sdk/examples/monitor-production-responses-to-user-queries) guide for an example.
</Note>
