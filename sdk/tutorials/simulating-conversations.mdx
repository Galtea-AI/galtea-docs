...
class MyAgent(galtea.Agent):
    ...
    def call(self, input_data: galtea.AgentInput) -> galtea.AgentResponse:
        ...
        # Get the retrieval context from your RAG if needed
        # retrieval_context = generate_retrieval_context(user_message)
...
evaluation_tasks = galtea_client.evaluation_tasks.create(
    session_id=session.id,
    metrics=["Factual Accuracy", "Role Adherence"],  # Replace with your metrics
)
