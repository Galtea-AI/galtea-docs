...
conversation_turns = [
    ...
    {'role': 'assistant', 'content': "Your order #123 has shipped."},
    {'role': 'user', 'content': "When will it arrive?"},
    {'role': 'assistant', 'content': "It is expected to arrive in 3-5 business days."}
]
...
# We iterate over assistant turns assuming a user-assistant-user-assistant structure
for i in range(1, len(conversation_turns), 2):
    user_turn = conversation_turns[i-1]
    assistant_turn = conversation_turns[i]
    ...
    relevance_score = score_turn_relevance(assistant_turn["content"])
...
print(f"Logged and evaluated {len(conversation_turns) // 2} turns for session {session.id} with custom scores.")
```