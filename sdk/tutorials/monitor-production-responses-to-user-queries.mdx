...
from galtea.application.services.evaluation_task_service import EvaluationMetric
import os
...
def handle_user_query(user_query, retrieval_context=None):
    ...
    galtea.evaluation_tasks.create_single_turn(
      ...
      is_production=True,
      metrics=[EvaluationMetric(name="coherence"), EvaluationMetric(name="answer-relevancy"), EvaluationMetric(name="faithfulness")],
      input=user_query,
...
VERSION_ID = "YOUR_ACTIVE_VERSION_ID"
METRICS_TO_EVALUATE = [
    EvaluationMetric(name="conversation-relevancy"),
    EvaluationMetric(name="knowledge-retention")
]
