---
title: 'Run Test-Based Evaluation Tasks'
...
<Info>
  A [session](/concepts/product/version/session) is created **implicitly** the first time you run an evaluation task for a specific `version_id` and `test_id` combination. You do not need to create it manually.
</Info>
