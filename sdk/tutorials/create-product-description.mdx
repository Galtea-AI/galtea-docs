...
## Product Context: Extended Product Descriptions
For maximum generation effectiveness, especially for security testing, you need to provide additional details for **capabilities, inabilities and security boundaries**. As a rule of thumb it is better to be explicit and structured in these extended definitions, listing each enhanced definition with bullet point lists.

In the following section we provide the definitions for each and enumerate specific examples based on the following product description:
