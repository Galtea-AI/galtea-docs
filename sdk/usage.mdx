...
### Launching Evaluation Tasks

[Evaluation tasks](/concepts/product/version/session/evaluation-task) assess the performance of a [product](/concepts/product) [version](/concepts/product/version) against [test cases](/concepts/product/test/case) using specific [metric types](/concepts/metric-type). They are now directly linked to [sessions](/concepts/product/version/session) that contain all inference results.
...
```python
# First, get all sessions for the product
sessions = galtea.sessions.list(product_id=product.id)

# Iterate through all product sessions
for session in sessions:
    # Retrieve the evaluation tasks for this session
    evaluation_tasks = galtea.evaluation_tasks.list(session_id=session.id)
