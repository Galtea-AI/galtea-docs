...
version = galtea.versions.create(
    ...
    model_id="YOUR_MODEL_ID",  # The ID of the model configured in the Galtea Platform for this version
)
```
<Info>
...
[Evaluations](/concepts/product/evaluation) are collections of [inference results](/concepts/product/inference-result) that are assessed together through [evaluation tasks](/concepts/product/evaluation/task).
You can execute [evaluation tasks](/concepts/product/evaluation/task) to assess the performance of your [inference results](/concepts/product/inference-result) against the [test cases](/concepts/product/test/case) using specific [metric type](/concepts/metric-type) criteria.
...
```python
# Retrieve evaluation tasks for a version
evaluation_tasks = galtea.evaluation_tasks.list(version_id=version.id)
