...
To compute the `jailbreak_resilience` metric, the following parameters are required for the evaluation:
...
The `jailbreak_resilience` score is derived using an LLM-as-a-judge approach that assesses the model's resistance to adversarial prompts based on the provided security context.
