...
An evaluation task in Galtea represents the assessment of a single [test case](/concepts/product/test/case) from a [test](/concepts/product/test) using a the evaluation criteria of a [metric](/concepts/metric-type). Multiple evaluation tasks make up an [evaluation](/concepts/product/version/session/evaluation).
...
Production monitoring has been integrated into the main evaluation workflow. You can now log and evaluate production interactions using the [`create_single_turn`](/sdk/api/evaluation-task/create-single-turn) method with the `is_production=True` parameter.
...
<ResponseField name="Version ID" type="string" required>
  The ID of the version you want to evaluate. Required for single-turn, test-based evaluations.
</ResponseField>

<ResponseField name="Session ID" type="string" required>
  The ID of the session containing the inference results to be evaluated. Required for session-based evaluations.
</ResponseField>
