  ...
  <Tab title="Generate from Ground Truth">
    <Steps>
      ...
      <Step title="Generate the test">
        Galtea will process the Ground Truth and generate a *Test File* and *Test Cases* with adversarial inputs for you
      </Step>
    </Steps>
  </Tab>
...
<Note>
  This structure is the required by Galtea in order to automatically generate the [test cases](/concepts/product/test/case) for your [test](/concepts/product/test). If that format is not provided, you can still [manually create the test cases](http://localhost:3000/sdk/api/test-case-service#create-test-case).
</Note>
