...
-   **`product_description`**: An overview of the product.
-   **`product_capabilities`**: What the product is designed to do.
-   **`product_inabilities`**: Topics or functions the product is not capable of handling.
-   **`product_security_boundaries`**: Actions or information the product can access but is explicitly forbidden from sharing.
