...
- **Evaluator Model:** This is an LLM used by Galtea to perform "LLM-as-a-judge" evaluations for non-deterministic [Metric Types](/concepts/metric-type). You select an Evaluator Model when creating a custom metric with a `judge_prompt`.
