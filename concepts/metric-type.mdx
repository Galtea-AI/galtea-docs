...
| **[Custom Judge](/concepts/metric-type/custom-judge)** | RAG |  A flexible LLM-as-a-judge evaluation metric designed to assess model outputs based on customizable product rubrics, domain-specific behavior expectations, and grounded context.  |
...
you can also provide a custom judge by passing a prompt to control even further how the evaluation is performed by our evaluator.
Example:

```python
judge_prompt = """
  Judge the following output based on the coherence between the input, the expected output and the actual output.
...
You can include some evaluation data as placeholders so that the prompt is built correctly in execution time.
