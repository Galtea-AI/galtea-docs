...
<ResponseField name="Test Type" type="Enum" required>
  The type of test this metric is designed for. This indicates the evaluation 
  context where the metric should be used.
  
  Possible values:
  - `QUALITY` - For quality testing and accuracy evaluations
  - `RED_TEAMING` - For security and safety testing
  - `SCENARIOS` - For conversational and multi-turn evaluations
