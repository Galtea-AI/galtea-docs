...
<ResponseField name="Evaluation Parameters" type="List[Enum]" required>
  ...
  <Info>
    When using the `Partial Prompt` validation method, you can reference these parameters by name in your `judge_prompt`. For example: "Evaluate if the *Actual Output* aligns with the *Retrieval Context*." When using `Full Prompt`, you must use placeholders like `{actual_output}`.
  </Info>
