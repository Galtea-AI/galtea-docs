...
| **[Jailbreak Resilience](/concepts/metric/jailbreak-resilience)** | Red Teaming | A previous version of the [Jailbreak Resilience v2](/concepts/metric/jailbreak-resilience-v2) Metric. |
| **[Jailbreak Resilience v2](/concepts/metric/jailbreak-resilience-v2)** | Red Teaming | Evaluates the ability of an LLM-based product to resist attempts at breaking or manipulating its intended behavior. |
...
<ResponseField name="Validation method" type="Enum" required>
  ...
  <Tabs>
    ...
    <Tab title="Partial Prompt">
      This method simplifies prompt creation. You provide only the core evaluation criteria or rubric in the judge prompt. Then, you select the necessary data (like `input`, `context`, etc.) from the **Evaluation Parameters** list. Galtea dynamically constructs the final prompt by prepending the selected data to your criteria, ensuring a consistent structure.
    </Tab>
    <Tab title="Self Hosted">
      This method is for deterministic metrics that you score locally using the SDK's `CustomScoreEvaluationMetric`. No evaluation prompt is sent to Galtea. Instead, your custom logic runs on your infrastructure, and the resulting score is uploaded to the platform for tracking and analysis.
    </Tab>
  </Tabs>
</ResponseField>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>Scenarios tests (Full Prompt only)</td>
      </tr>
    </tbody>
  </table>
...
2. **Score Computation**: The model assigns a score based on the given scale of the rubrics, which is then normalized to **0–1** via token-probability weighting
