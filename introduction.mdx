...
<Steps>
  ...
  <Step title="Run Evaluation Tasks">Execute [Evaluation Tasks](/concepts/product/evaluation/task) to assess your [Inference Results](/concepts/product/inference-result) using specific [Metric Types](/concepts/metric-type). [Evaluations](/concepts/product/evaluation) are automatically created to group related results</Step>
  <Step title="Analyze Results">Review and compare the performance across different [Versions](/concepts/product/version), [Sessions](/concepts/product/session), and time periods using the built-in analytics tools</Step>
  <Step title="Iterate">Develop a new [Version](/concepts/product/version) of your [Product](/concepts/product) and repeat the process to compare performance improvements</Step>
</Steps>
