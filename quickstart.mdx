...
As you click on **Products > Create New Product**, you will be presented with the product onboarding form. The stored metadata is used mostly for reporting purposes, and the description is used during the process of generating synthetic test data when needed.

![product onboarding](/images/product-onboarding.png)

## Install the SDK and connect via an API key

Once you have created your product, you can conveniently follow the rest of the process using the SDK. To do this, you will need your API key, which can be obtained from **Settings > Generate API Key**.

![Get API key](/images/api-key.png)

After acquiring the API key, install the Galtea library in your virtual environment:

```sh
pip install galtea
```

Once installed, you can verify the connection to the platform:
...
This should return a list of products that you have created within your organization. For now, you have only one product, and we have retrieved the ID of the last created product, stored in the variable `new_product_id`.
...
evaluation_result = galtea.evaluation_tasks.get(evaluation_tasks[-1].id)
print((evaluation_result.score, evaluation_result.reason))
```
