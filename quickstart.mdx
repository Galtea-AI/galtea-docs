...
# Load your test cases (assuming 'test' object was created in step 4)
...
for test_case in test_cases:
    ...
    galtea.evaluation_tasks.create(
        metrics=[metric.name], # Use the created metric's name
        evaluation_id=evaluation.id,
        test_case_id=test_case.id, # Use test_case_id
        actual_output=actual_output
    )
```

<Note>In this example, `your_product_function` is a placeholder for your model's inference logic. It should take the input and context from the `test_case` and return the model's output.</Note>
