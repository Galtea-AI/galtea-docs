...
Navigate to **[Products](https://platform.galtea.ai/) > Create New Product** and complete the product onboarding form. The product description is particularly important as it may be used during the generation of synthetic test data.
...
```
<Info>
  All `optional_props` can be found in the [Create Version](/sdk/api/version/service#create-version) API documentation.
</Info>

<Note>
  The product's ID can be found in the product's page in the [Galtea platform](https://platform.galtea.ai/) or by [listing the products](/sdk/api/product/service#listing-products) using the SDK.
</Note>
...
<CardGroup cols={2}>
  <Card title="Product Service API" icon="swatchbook" href="/sdk/api/product/service">
    Learn about all product properties and management capabilities
  </Card>
...
metric_self_accuracy = galtea.metrics.create(
    ...
    evaluation_params=["input", "expected output", "actual output"] # Ensure these match EvaluationParams values
)
```
...
<Note>
  In this example, `your_retriever_function` is a placeholder for your RAG system's retrieval logic (if applicable), and `your_product_function` is a placeholder for your model's inference logic. `your_product_function` should take the input, context (from test case), and retrieval_context (if any) and return the model's output string.
</Note>
