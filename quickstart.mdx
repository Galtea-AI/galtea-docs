...
With **Galtea**, you can enhance AI reliability, mitigate risks, streamline compliance, and accelerate time to market registration. This guide will walk you through the essential steps to get started with basic evaluation and track the reliability of your LLM-based products.

## Creating a Product

The first step in tracking the quality and reliability of your LLM-based product is to create a product in the dashboard. To do this, you will need to log in with your credentials.

## Registering a Version

One of the key advantages of the **Galtea platform** is the ability to track and compare different versions of an LLM-based product. A new version can include new prompts, different RAG parameters, or an entirely new end-to-end product with updated components. Before launching an evaluation, it is essential to register a new version on the platform.
...
```
<Note>
  The type of the tests could be `QUALITY` or `RED_TEAMING`. They have different formats and different uses. For more information check the relevant documentation on [quality](concepts/product/test/quality-tests) and [red teaming](concepts/product/test/red-teaming-tests).
</Note>
...
The metrics of galtea rely on LLM-as-a-Judge methodology and specifically uses the [GEval framework](https://doi.org/10.48550/arXiv.2303.16634). For this tutorial we will define our own metrics tailored to our specific use case. 
...
By doing this, we register the metric in the platform, in order for it to be reused later in the future for different tests.

## Launching an Evaluation

Finally, you are ready to launch an evaluation. The process begins by creating an evaluation:
...
Now you can see your results in the platform, within the evaluations tab or by doing:
...
Congratulations! You did your first evaluation with **Galtea**, one of many evaluations to come. If you have any questions, you can contact us in [support@galtea.ai](mailto:support@galtea.ai).