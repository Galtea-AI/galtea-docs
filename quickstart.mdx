...
<Note>
  **Metrics Format:** We recommend using the MetricInput dictionary format `[{"name": "metric_name"}]` for metrics. While the legacy string format `["metric_name"]` is still supported for backward compatibility, it will be deprecated in a future release. The dictionary format also allows you to pass pre-computed scores using `{"name": "metric_name", "score": 0.85}`.
</Note>
