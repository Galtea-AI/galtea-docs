...
- **Link Metrics to Specific Models**: You can now associate a [Metric](/concepts/metric) with a specific evaluator model (e.g., "GPT-4.1"). This ensures consistency across evaluation runs and allows you to use specialized models for certain metrics.
...
To better support applications that handle large documents or complex queries, we have increased the maximum character size for evaluation inputs and outputs to 250,000 characters.
