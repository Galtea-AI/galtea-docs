...
1. **Implicit Evaluation Creation**: You no longer need to explicitly call `galtea.evaluations.create()` before creating evaluations.

2. **`evaluations.create()` Repurposed**: The old `evaluations.create()` method has been replaced by the new `create_single_turn()` method for test-based evaluations. The name `create()` is now used exclusively for the new session-based workflow to evaluate all turns in a conversation.
...
# 3. Create evaluations using evaluation_id
...
3. Rename `galtea.evaluations.create()` calls to `galtea.evaluations.create_single_turn()`, **remove** the `evaluation_id` parameter, and **add** the `version_id` parameter.
